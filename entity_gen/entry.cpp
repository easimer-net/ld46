...
// TODO(danielm): handle arrays like Static_Prop::pszSpritePath
...
static Table_Definition ParseTable(Token_Stream_Iterator& it) {
    ...
    def.var_name = def.name;
    std::transform(def.name.begin(), def.name.end(), def.var_name.begin(), std::tolower);
    ...
    if (hFile != NULL) {
        ...
        for (auto& table : tables) {
            ...
            for (auto& field : table.fields) {
                ...
                bHasTempField |= (field.flags & k_unFieldFlags_Temporary) != 0;
            }

            if (bHasTempField) {
                fprintf(hFile, "    void ResetTransients() {\n");
                for (auto& field : table.fields) {
                    if ((field.flags & k_unFieldFlags_Temporary) != 0) {
