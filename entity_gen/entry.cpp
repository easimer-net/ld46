...
static void GenerateHeaderFile(String const& pszPath, Vector<Table_Definition> const& tables) {
    ...
    if (hFile != NULL) {
        ...
        fprintf(hFile, TAB "void Clear() { \n");
        for (auto& table : tables) {
            if (table.name != "Entity") {
                fprintf(hFile, TAB2 "%s.clear();\n", table.var_name.c_str());
            } else {
                fprintf(hFile, TAB2 "entities.clear();\n");
            }
        }
        fprintf(hFile, TAB "}\n\n");

        fprintf(hFile, TAB "void DeleteEntity(Entity_ID i) {\n");
        fprintf(hFile, TAB2 "assert(i < entities.size());\n");
        for (auto& table : tables) {
            if (table.name != "Entity") {
                fprintf(hFile, TAB2 "%s.erase(i);\n", table.var_name.c_str());
            }
        }
        fprintf(hFile, TAB2 "entities[i].bUsed = false;\n");
        fprintf(hFile, TAB "}\n");
